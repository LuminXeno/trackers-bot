...
import { parseSchedule, dayTracker } from "../lib/icsParser"
import { dayMapping } from "../lib/icsParser"
...
export const schedule: Machi = {
  data: (new SlashCommandBuilder()).setDescription("Schedule related commands")
    .addSubcommand(
      ...
      command => command
        .setName("current")
        .setDescription("Check Your Current/Next Class!")
    ),
  ...
  subCommands: {
    ...
    current: async (interaction: ChatInputCommandInteraction) => {
      ...
      //testdays
      // Notes: use moment.js for reversing format to 24h time for inidicual classes
      //const testDay = 2

      const userSchedule = userData.scheduleData.schedule as ReturnType<typeof parseSchedule>
      let currentSchedule = []
      for(let _ = 0; _ < userSchedule.length; _++){
        console.log(currentDay.getDay())
        if(userSchedule[_].days.includes(dayTracker[currentDay.getDay()])){
          currentSchedule.push(userSchedule[_])
        }
      }
      interaction.reply({
        embeds: [{
          ...
          title: `These are ${interaction.user.username} classes for today!`,
          // description: `User schedule for ${userSchedule}`,
          fields: displayScheduleAsFields(currentSchedule),
          color: HEX.GREEN
        }],
        ephemeral: true
      })
